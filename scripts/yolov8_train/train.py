...
class YOLOv8Trainer:
    def __init__(self, base_dir='yolo_experiments'):
        ...
        self.model_dir = self.base_dir / 'v8'
    ...
    def create_run_directory(self, dataset_name, model_size, batch_size, epochs):
        ...
        run_name = f"yolov8{model_size}_batch{batch_size}_epoch{epochs}_{timestamp}"
    ...
    def train_model(self, data_yaml_path, dataset_name, model_size='s', batch_size=16, epochs=100, img_size=640):
        ...
        try:
            ...
            logging.info(f"Starting training with YOLOv8{model_size}")
            model.train(**args)
...
def main():
    ...
    MODEL_CONFIGS = [
        {'size': 'n', 'batch_size': 32},
        {'size': 's', 'batch_size': 16},
    ...
    trainer = YOLOv8Trainer(base_dir=r'C:\Users\Eduar\Documents\masters\cap5415\solar_project\scripts\yolo_experiments')
    ...
    for dataset_name, data_yaml_path in DATASETS.items():
        for config in MODEL_CONFIGS:
            try:
                ...
                logging.info(f"Model: YOLOv8{config['size']}")
